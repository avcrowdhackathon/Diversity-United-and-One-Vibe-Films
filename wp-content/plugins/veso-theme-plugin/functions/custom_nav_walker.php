...
class Veso_Nav_Walker extends Walker_Nav_Menu {
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if($item->object == 'veso_mega_menu') {
			...
			$i = 0;
			$empty_headers = false;
			if(get_post_meta($item->object_id, 'veso_mega_menu_column_1_header', true) == '' && get_post_meta($item->object_id, 'veso_mega_menu_column_2_header', true) == '' && get_post_meta($item->object_id, 'veso_mega_menu_column_3_header', true) == '' && get_post_meta($item->object_id, 'veso_mega_menu_column_4_header', true) == '' && get_post_meta($item->object_id, 'veso_mega_menu_column_5_header', true) == '') {
				$empty_headers = true;
			}
			if($mega_menu_1) { $i++; }
			if($mega_menu_2) { $i++; }
			...
			if($i > 0) {
				...
				$output .= '<div class="sub-menu mega-menu row" data-width="'.get_post_meta($item->object_id, 'veso_mega_menu_width', true).'">';
				...
				if($image) {
					$output .= '<div class="large-4 mega-menu-content ver-'.$ver_position.'" style="background: url('.$image.'); background-size: cover; background-position: center center;"><img style="display: none;" src="'.$image.'" alt=""/><div class="mega-menu-content-overlay" style="background-color: '.$overlay_color.'"></div><div class="mega-menu-content-inner">'.apply_filters('the_content', $post_content).'</div></div>';
					$output .= '<div class="large-8"><div class="mega-menu-inner">';
				} else {
					$output .= '<div class="large-12"><div class="mega-menu-inner">';
				}
				
				$output .= '<div class="row">';
				if($header_text) {

					$output .= '<div class="large-12 columns"><h4>'.$header_text.'</h4></div></div><div class="row">';
				}
					...
					if($menu_post) {
						...
						if($empty_headers == false) {
							if(get_post_meta($item->object_id, 'veso_mega_menu_column_'.$index.'_header', true)) {
								$header = '<h6>'.get_post_meta($item->object_id, 'veso_mega_menu_column_'.$index.'_header', true).'</h6>';
							} else {
								$header = '<h6 class="empty-header">Empty</h6>';
							}
							$output .= '<li>'.$header.'</li>';
						}
