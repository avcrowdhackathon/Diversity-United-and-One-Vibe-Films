...
class acf_field_typography extends acf_field {
	...
	function __construct() {
		...
		// $json = wp_remote_fopen("{$dir}gf.json");
	...
	function render_field_settings( $field ) {
		...
		acf_render_field_setting( $field, array(
			'label'			=> __('Show Additional Font Weight ?','acf-typography'),
			'type'			=> 'radio',
			'layout'		=>  'horizontal',
			'name'			=> 'show_font_additional_weight',
			'choices'		=>	array(
								1	=>	__('Yes','acf-font-awesome'),
								0	=>	__('No','acf-font-awesome')
							)
	...
	function render_field( $field ) {
        ...
        if( empty($field['value']) ){
            ...
            $field['value']['font-weight-multi'] = $field['font-weight-multi'];
			...
			echo "<div class='clearfix'>";
				...
				if ($field['show_font_style']){
					echo '<div class="acf-typography-subfield acf-typography-font-style">';
						...
						echo '<select name="' . $field['name'] . '[font_style]" id="' . $field['key'] . '-font-style"  class="js-select2 font-style" multiple="multiple">';
				...
				if ($field['show_font_weight'] & $field['show_font_familys']) {
					echo '<div class="acf-typography-subfield acf-typography-font-weight">';
						echo '<label class="acf-typography-field-label" for="'. $field['key'] .'">Load Additional Font Weight</label>';
						// var_dump($field_value['font-weight-multi']);
						echo '<input name="' . $field['name'] . '[font-weight-multi]" id="' . $field['key'] . '" value="' . $field_value['font-weight-multi'] . '" class="select2-container font-weight select2-weight-multi" type="hidden" />';
					echo '</div>';
				}
	...
	function validate_value( $valid, $value, $field, $input ){
	    if ($field['required']) {
		    if (empty($value['font-family']) || empty($value['font-weight']) || empty($value['backupfont']) || empty($value['text_align'])
		    	...
		    	if( empty($value['font-weight-multi']) & $field['show_font_weight']){
		    		$txt .= __('font weight multi, ','acf-typography');
		    		$set = 1;
		    	}
